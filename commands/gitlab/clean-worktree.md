---
description: Cleanup worktree and tmux session for a completed GitLab issue
...
6. **Confirm cleanup**
   - Display what was cleaned up (tmux session, worktree)
...
if [ "$TMUX_CLEANED" = true ] || [ "$WORKTREE_CLEANED" = true ]; then
    ...
    echo "  No tmux session or worktree found for issue #$ISSUE_NUMBER"
fi
...
### Remove both worktree and tmux session:
```bash
git worktree remove ../worktrees/<branch-name>
tmux kill-session -t <issue-number>-develop
```
...
- **Graceful failure**: If tmux or worktree doesn't exist, the command will skip that step and continue
...
4. **Remote branch**: Consider deleting the remote branch after cleanup if the issue is merged
